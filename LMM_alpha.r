...
x<-correlas[["r"]]
write.csv(file="Stats_s_26_07_2023.csv", x)
x<-correlas[["P"]]
write.csv(file="Stats_sp_26_07_2023.csv", x)
...
lmm2.1_c<-lmer(formula = shannon_entropy ~ bio19 + pre + tm_max + elevation + 
                 bio8 + bio12 + bio17 + bio1 + bio10 + bio18 + bio6 + bio2 +
                 morphotype + (1|Study) + Family, REML=F, data = na.omit(infossm_c))
...
write.csv(tabla_VIF, "table_VIF_lmm_26_07_2023.csv")
...
write.csv(tabla_VIF2, "table_VIF_lmm2_26_07_2023.csv")
...
save(infossm,lmm2.1_s, file = "model_data_26_07_2023.RData")
